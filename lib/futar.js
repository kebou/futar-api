...
const FUTAR_URL = 'http://futar.bkk.hu/bkk-utvonaltervezo-api/ws/otp/api/where/';
...
class Futar {
    ...
    stopsForLocation(opts, cb) {
        return new Promise((resolve, reject) => {
            if (!opts || !opts.lat || !opts.lon || !opts.radius) {
                let err = new Error('A lat, lon és radius megadása kötelező!');
     ...
     * @param {string} query a keresést tartalmazó string
     * @param {Futar~requestCallback} [cb] a választ kezelő callback függvény
     * @return {Promise} Promise, ha nincs callback paraméter
     */
    search(opts, cb) {
        return new Promise((resolve, reject) => {
            if (!opts || (typeof opts === 'object') && !opts.query) {
                let err = new Error('A query megadása kötelező!');
                err.name = 'InvalidArgumentError';
                throw err;
            }
    ...
    planTrip(opts, cb) {
        return new Promise((resolve, reject) => {
            if (!opts || !opts.fromLat || !opts.fromLon || !opts.toLat || !opts.toLon) {
                let err = new Error('A fromLat, fromLon, toLat és toLon megadása kötelező!');
                err.name = 'InvalidArgumentError';
                throw err;
            }
    ...
    _validateStop(opts) {
        if (!opts || (typeof opts === 'object') && !opts.stopId) {
            let err = new Error('A stopId megadása kötelező!');
            err.name = 'InvalidArgumentError';
            throw err;
        }
    ...
    _validateRoute(opts) {
        if (!opts || (typeof opts === 'object') && !opts.routeId) {
            let err = new Error('A routeId megadása kötelező!');
            err.name = 'InvalidArgumentError';
            throw err;
        }
